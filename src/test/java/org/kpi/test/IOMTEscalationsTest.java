...
public class IOMTEscalationsTest {
	...
	public void checkIOMTElements() {
		try {
			...
			if(flag) {
				...
				Thread.sleep(5000);
				Utility.findElement(driver, "iomt_EscalationItemTile").click();
			}
			else {
			...
			Thread.sleep(5000);
			Assert.assertTrue(Utility.isElementPresent(driver, "IOMTDetailBackIcon"),
					"Escalations Tab Esclations Tab Back Icon Not Found");
			Assert.assertTrue(Utility.isElementPresent(driver, "IOMTDetailTitle"),
					"Escalations Tab Esclation Tab Header Service Type Text Not Found");
			Assert.assertTrue(Utility.isElementPresent(driver, "IOMTDetailSubTitle"),
					"Escalations Tab Esclation Tab Header Name Text Not Found");
	...
	@Test(priority = 1)
	public void checkIOMTCTElements() {
		try {
			Thread.sleep(5000);
	...
	@Test(priority = 2)
	public void checkIOMTInProgress() {
		try {
			Thread.sleep(5000);
			TouchAction tap = new TouchAction(driver);
			...
			Thread.sleep(5000);
			if(flag) {
				if(Utility.isElementPresent(driver, "iomt_EscalationItemTile")) {
					count = 1;
				}
				else {
					count = 0;
	...
	@Test(priority = 3)
	public void checkIOMTInProgressCT() {
		try {
			Thread.sleep(5000);
			TouchAction tap = new TouchAction(driver);
			tap.tap(200, 200).perform();
			/*if(count >= 1) {
				...
				Utility.findElement(driver, "Communication thread").click();
				Thread.sleep(5000);
				Assert.assertTrue(Utility.isElementPresent(driver, "CTAttachIcon"), "File Attachement in IOMT Escalations Communication Thread Not Found");
				Assert.assertTrue(Utility.isElementPresent(driver, "Type a message"), "Input Box in IOMT Escalations Communication Thread Not Found");
				Assert.assertTrue(Utility.isElementPresent(driver, "CTCharcterCount"), "Character Count Text in IOMT Escalations Communication Thread Not Found");
				Thread.sleep(5000);
	...
	@Test(priority = 4)
	public void checkIOMTMarkAsRemediated() {
		try {
			Thread.sleep(5000);
			TouchAction tap = new TouchAction(driver);
			tap.tap(200, 200).perform();
			/*if(count >= 1) {
				...
				if (Utility.isElementPresent(driver, "EsclationTabAssignButton")) {
					...
					Thread.sleep(3000);
					Utility.findElement(driver, "EsclationTabAssignedToSearchInput").sendKeys("sreedar");
					Thread.sleep(3000);
					if(flag) {
	...
	@Test(priority = 5)
	public void checkIOMTEscalationsFeedback() {
		try {
			if(count >= 1) {
				Thread.sleep(5000);
				TouchAction tap = new TouchAction(driver);
